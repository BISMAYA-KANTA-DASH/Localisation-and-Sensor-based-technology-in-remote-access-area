...
def generate_sensors(num_sensors, radius=10):
    angles = np.linspace(0, 2 * np.pi, num_sensors - 1, endpoint=False)
    x = radius * np.cos(angles) + np.random.uniform(-0.5, 0.5, num_sensors - 1)
    y = radius * np.sin(angles) + np.random.uniform(-0.5, 0.5, num_sensors - 1)
    sensors = np.column_stack((x, y))
    
    # Add one sensor outside the circle
    outside_sensor = np.array([[radius * 1.5, radius * 1.5]])
    sensors = np.vstack([sensors, outside_sensor])
