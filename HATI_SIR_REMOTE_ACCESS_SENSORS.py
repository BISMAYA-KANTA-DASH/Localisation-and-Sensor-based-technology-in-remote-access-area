...
# Generate random sensor positions sprinkled within a circular area
def generate_sensors(num_sensors, radius=10):
    sensors = []
    for _ in range(num_sensors - 1):  # One sensor will be outside the circle
        r = np.sqrt(np.random.uniform(0, 1)) * radius  # Random radius within the circle
        theta = np.random.uniform(0, 2 * np.pi)  # Random angle
        x, y = r * np.cos(theta), r * np.sin(theta)
        sensors.append([x, y])
    
    # Add one sensor outside the circle
    r_out = np.random.uniform(radius * 1.2, radius * 1.8)  # Outside the circle
...
def connect_sensors(sensors):
    ...
    # Ensure the outside sensor connects to two closest inside sensors
